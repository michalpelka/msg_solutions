...
def deriveAnalitics(I1, D1, I2, xi, K, dI2x, dI2y):
    assert (len(xi)==6)
    J = []
    cx = K[0][2]; cy= K[1][2]
    fx = K[0][0]; fy= K[1][1]
...
with open("/tmp/optim.txt", 'w') as log:
    for k in range (PYRAMID_LEVELS, 0, -1):
        ...
        for i in range (0,5):
            ...
            J = deriveAnalitics(Id1, Dd1, Id2,sp, Kd1, dI2x, dI2y)
            ...
            err = np.sum(r)
            delta_sp = np.linalg.inv(-(np.transpose(J_trim) @ J_trim)) @ np.transpose(J_trim) @ r_trim
